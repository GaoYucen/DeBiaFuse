...
def train_one(path, args):
    ...
    windows = [make_decomposed_windows(z, args.look_back, args.horizon, args.n_high, s, e) for s, e in ((0, a), (a, b), (b, len(z)))]
    ...
    persistence = np.repeat((l_te[:, -1] + h_te[:, -1, :].sum(-1))[:, None], args.horizon, axis=1)
    residual = args.target == "residual"
    if residual:
        base_tr, base_va, base_te = l_tr[:, -1], l_va[:, -1], l_te[:, -1]
        total_tr, total_va, total_te = total_tr - base_tr[:, None], total_va - base_va[:, None], total_te - base_te[:, None]
    ...
    if residual: pred += base_te[:, None]; true = scaler.inverse_transform(total_te + base_te[:, None]); pred_raw = scaler.inverse_transform(pred)
...
def main():
    root = Path(__file__).resolve().parents[1]; ap = argparse.ArgumentParser(); ap.add_argument("--data-dir", default=str(root / "data" / "Hongfu" / "deflection")); ap.add_argument("--output", default="results/debiafuse_hongfu_seed42.json"); ap.add_argument("--look-back", type=int, default=24); ap.add_argument("--horizon", type=int, default=6); ap.add_argument("--n-high", type=int, default=3); ap.add_argument("--epochs", type=int, default=50); ap.add_argument("--patience", type=int, default=15); ap.add_argument("--batch-size", type=int, default=32); ap.add_argument("--seed", type=int, default=42); ap.add_argument("--target", choices=["direct", "residual"], default="residual"); ap.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu"); args = ap.parse_args(); torch.manual_seed(args.seed); np.random.seed(args.seed)
    rows = []; quality = []
