"""TimeMixer++ baseline for BRIDGE-V3-02 using the PyPOTS ICLR-2025 backbone.

The official-style backbone is trained inside the same masked-Huber / validation-only
loop used for other Hongfu neural baselines, avoiding high-level trainer differences.
"""
...
PYPOTS_SHA='53b3eac34be9491ac3f28e65ee1993436e9318af'; PROTOCOL='bridge-v3-e2-timemixerpp-pypots-backbone-v1-149to30'; CACHE=Path('/workspace/code-bridge/repro/20260914-r2/cache')
...
def run(a):
 ...
 for ep in range(1,a.epochs+1):
  ...
  for x,y,m in loader:
   x,y,m=x.to(device),y.to(device),m.to(device); opt.zero_grad(set_to_none=True); pr=model.forecast(x.unsqueeze(-1),None).squeeze(-1); loss=masked_mean(nn.functional.huber_loss(pr,y,reduction='none'),m); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step(); tot+=float(loss.detach()); nb+=1
  model.eval()
  with torch.no_grad(): val=float(masked_mean(torch.abs(model.forecast(vx.unsqueeze(-1),None).squeeze(-1)-vy),vm))
 ...
 with torch.no_grad():
  for x,_,_ in DataLoader(ds(p['test']),batch_size=64): outs.append(model.forecast(x.to(device).unsqueeze(-1),None).squeeze(-1).cpu().numpy())
 ...
 cfg={'n_steps':149,'n_pred_steps':30,'task':'long_term_forecast','n_layers':2,'d_model':32,'d_ffn':32,'top_k':5,'n_heads':1,'n_kernels':3,'dropout':.1,'channel_mixing':True,'channel_independence':True,'downsampling_layers':2,'downsampling_window':2,'use_norm':True,'batch_size':32,'lr':1e-3,'epochs':a.epochs,'patience':a.patience}
 res={'status':'success','protocol':PROTOCOL,'dataset':a.sensor,'model':'timemixerpp','seed':a.seed,'raw_history_budget':149,'horizon':30,'cache_sha256':digest(CACHE/f'{a.sensor}.npz'),'observed':sc,'model_info':{'vendor':'WenjieDu/PyPOTS','vendor_commit':PYPOTS_SHA,'parameter_count':sum(z.numel() for z in model.parameters()),'best_epoch':be,'validation_metric':best,'config':cfg,'train_seconds':train_seconds,'inference_seconds':infer_seconds}}
